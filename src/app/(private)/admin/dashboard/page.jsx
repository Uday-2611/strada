...
const page = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      const { data: bookings } = await client.from('bookings').select('id, status, created_at, user_id, vehicle_id, total_amount').eq('status', 'confirmed').order('created_at', { ascending: false }).limit(5);

      const { count: activeCount } = await client.from('bookings').select('id', { count: 'exact', head: true }).eq('status', 'confirmed');

      const { count: totalCount } = await client.from('bookings').select('id', { count: 'exact', head: true });
